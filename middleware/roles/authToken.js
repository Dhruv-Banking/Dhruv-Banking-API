...
function authGetRoleSpecificUsers(req, res, next) {
    ...
    if (role === roleData.admin || role === roleData.basic || role === roleData.god) {
        next();
    } else {
...
function updateUser(req, res, next) {
    ...
    if (role === roleData.basic || role === roleData.admin || role === roleData.god) {
        next();
    } else {
        // If not, then we know the user is bad.
        res.status(401);
...
module.exports = {
    authRoleGetAllUsers, authGetRoleSpecificUsers, updateUser, checkingToSavings, savingsToChecking, toAnotherUser,
};
